 ...
 * xclaw while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per xclaw instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to xclaw from inside the terminal)
