...
import { AGENT_TOOL_NAME, LEGACY_AGENT_TOOL_NAME } from '@xclaw/builtin-tools/tools/AgentTool/constants.js';
...
export function formatToolUseSummary(name: string, input: unknown): string {
  ...
  if (name === EXIT_PLAN_MODE_V2_TOOL_NAME) {
    return 'Review the plan in xclaw on the web';
  }
    ...
    return (
      <Dialog title="Stop ultraplan?" onCancel={() => setConfirmingStop(false)} color="background">
        <Box flexDirection="column" gap={1}>
          <Text dimColor>This will terminate the xclaw on the web session.</Text>
          <Select
            options={[
    ...
    >
      <Box flexDirection="column" gap={1}>
        ...
        <Select
          options={[
            {
              label: 'Review in xclaw on the web',
              value: 'open' as const,
            },
...
function ReviewSessionDetail({ session, onDone, onBack, onKill }: Omit<Props, 'toolUseContext'>): React.ReactNode {
  ...
  const options: { label: string; value: MenuAction }[] = completed
    ? [
        { label: 'Open in xclaw on the web', value: 'open' },
