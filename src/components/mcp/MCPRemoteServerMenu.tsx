  ...
  const handleClaudeAIAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        onComplete?.(
          'Authentication successful, but server still requires authentication. You may need to manually restart xclaw.',
        );
      } else {
        onComplete?.(
          'Authentication successful, but server reconnection failed. You may need to manually restart xclaw for the changes to take effect.',
        );
      }
    } catch (err) {
