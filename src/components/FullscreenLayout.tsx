...
type Props = {
  ...
  bottom: ReactNode;
  /** Left panel content (mode/tools info). Only shown when terminal >= 120 cols. */
  leftPanel?: ReactNode;
  /** Right panel content (accountability/code preview). Only shown when terminal >= 120 cols. */
...
export function FullscreenLayout({
  ...
  useLayoutEffect(() => {
    ...
    ink.onHyperlinkClick = url => {
      // Most OSC 8 links emitted by xclaw are file:// URLs from
  ...
  if (isFullscreenEnvEnabled()) {
    ...
    const showSidePanels = columns >= 100 && (leftPanel || rightPanel);
    const leftWidth = 20;
    const rightWidth = 25;
    const centerWidth = showSidePanels
      ? columns - (leftPanel ? leftWidth : 0) - (rightPanel ? rightWidth : 0)
      : columns;
    return (
      <PromptOverlayProvider>
        <Box flexDirection="row" flexGrow={1} overflow="hidden" width="100%">
          ...
          <Box flexDirection="column" flexGrow={1} width={centerWidth} overflow="hidden">
