...
function Install({ onDone, force, target }: InstallProps): React.ReactNode {
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'xclaw installation completed successfully', {
        display: 'system' as const,
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'xclaw installation failed', {
        display: 'system' as const,
      });
    }
  }, [state, onDone]);
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">Installing xclaw native build {state.version}...</Text>}
...
export const install = {
  ...
  name: 'install',
  description: 'Install xclaw native build',
