...
Langfuse 是一个开源的 LLM 可观测性平台，用于追踪、监控和调试 AI 应用的请求链路。xclaw 通过 OpenTelemetry (OTel) 桥接层将 Langfuse 集成到查询流程中，实现：
...
`convert.ts` 将 xclaw 内部的 Message 类型转换为 Langfuse 期望的 OpenAI 兼容格式：
