# xclaw Best (xclaw) — 全功能使用指南

本文档覆盖我们通过 13 个 PR 为 xclaw 恢复/新增的**全部功能**，按类别组织，每个功能包含说明、使用方法和示例。
...
### 说明
通过 WebSocket 远程控制 xclaw 会话。支持自托管私有部署。
...
### 说明
Daemon 模式允许 xclaw 作为后台长驻进程运行，管理多个 worker。
...
### 说明
同一台机器上的多个 xclaw 实例通过 UDS（Unix Domain Socket / Windows Named Pipe）自动发现并协作。首个启动的实例成为 main，后续自动注册为 sub。
