...
本指南说明如何将 Remote Control Server (RCS) 部署到私有环境，并通过 xclaw CLI 连接使用。

## 架构概览

```
┌──────────────────┐                    ┌──────────────────────┐
│  xclaw CLI  │ ◄── HTTP/SSE/WS ─►│  Remote Control      │
...
- 一台可被 xclaw CLI 和 Web 浏览器同时访问的服务器（物理机、VM、容器均可）
- [Docker](https://www.docker.com/)
- 启用 `BRIDGE_MODE` feature flag 的 xclaw 构建
...
- 查看已注册的运行环境（environment 模式），区分 ACP Agent 和 xclaw 类型
- 创建和管理会话
- 实时查看对话消息和工具调用
...
ACP session 在 Web UI 中显示品牌色标签，与普通 xclaw session 区分。
...
└──────────────────────────────────────────────────────────┘
 ...
 1. xclaw CLI 启动，设置环境变量指向自托管 RCS
