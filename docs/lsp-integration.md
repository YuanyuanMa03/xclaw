...
xclaw 内置了 Language Server Protocol (LSP) 集成，提供代码智能功能（跳转定义、查找引用、悬停信息、文档符号等）和被动的诊断反馈。

## 快速开始

### 1. 安装 LSP 插件

在 xclaw REPL 中使用 `/plugin` 命令搜索并安装 LSP 插件：
...
除了手动 `/plugin` 搜索安装外，xclaw 会在编辑文件时自动检测：
