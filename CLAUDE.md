...
> 基于 Claude Code Best (CCB) v2.0.4 改造。原项目为 Anthropic 官方 CLI 工具的逆向/反编译版本。

This file provides guidance to AI coding agents when working with code in this repository.

## Project Overview

xclaw — 超越人类与 AI 边界的 AI 编程 CLI 工具。基于 CCB 渐进式改造，在保留核心功能的同时添加模式系统、代码问责、记忆系统等特性。TypeScript strict mode is enforced — **`bun run precheck` 必须零错误通过**（包含 typecheck + lint fix + test）。
