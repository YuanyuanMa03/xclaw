...
This file provides guidance to xclaw (claude.ai/code) and other AI coding agents when working with code in this repository.

## Project Overview

This is a **reverse-engineered / decompiled** version of Anthropic's official xclaw CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced — **`bunx tsc --noEmit` must pass with zero errors**.
...
| `packages/cc-knowledge/` | xclaw 知识库（非 workspace 包） |
...
- **`src/services/acp/`** — ACP agent 实现，包含 `agent.ts`（AcpAgent 类）、`bridge.ts`（xclaw ↔ ACP 桥接）、`permissions.ts`（权限处理）、`entry.ts`（入口）。
